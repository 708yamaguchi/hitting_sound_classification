...
from classify_hitting_sound import ClassifyHittingSound
import rospy
...
class ClassifyHittingSoundROS:
    def __init__(self):
        ...
        self.chs = ClassifyHittingSound()
        self.chs.load_data(k=7)
    ...
    def cb(self, msg):
        ...
        if wave_strength > self.wave_strength_thre:
            class_name = self.chs.predict(spec_data)
            msg = String()
            msg.data = class_name
...
if __name__ == '__main__':
    rospy.init_node('classify_hitting_sound_ros')
    ClassifyHittingSoundROS()
    rospy.spin()
